...
4. Go to /admin/settings/audio-convert and choose which audio fields are to be converted, when the conversion should happen and what to do with the converted files. Individual field configurations might be done by going to Content Type -> Manage Fields -> Choose a field -> Configure
...
2. After installation, go to /admin/settings/audio-convert and enter the full path to the new FFMPEG program. Example: /usr/bin/ffmpeg
...
3. Go to /admin/settings/audio-convert and enter the full path to the new ffmpeg.exe program. Example: C:\wamp\apps\ffmpeg.exe
