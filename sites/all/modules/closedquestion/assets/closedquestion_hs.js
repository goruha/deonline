 ...
 */
Drupal.behaviors.closedQuestionHS = function (context) {
  ...
  for (var questionId in settings) {
    ...
    /* turn all elements with class "draggable" in startlist into jquery draggables
      */
    $(".cqDdDraggable").draggable({
      zIndex : 10000,
      containment : "parent"
    });
    $("#" + answerContainerId + " .cqDdDraggable").attr("questionId", questionId);
    ...
    for (var i = 0; i < length; i++){
      ...
      var draggable = $("#" + answerContainerId + " .cqDdDraggable[cqvalue=" + cqvalue + "]");
    ...
    answerContainer.droppable({
      ...
      drop: function(event, ui) {
        ...
        var coords = cqGetCenterCoordsHS($(this), ui.draggable);
    ...
    var createDraggableFunction = function(e) {
      ...
      if ( this.nodeName == 'MAP' ) {
        answerContainer = $('#' + questionId + 'answerContainer');
      }
      else {
      ...
      if (qsettings.maxChoices == undefined || numberOfRegisteredDraggables < qsettings.maxChoices) {
        ...
        $(newDrag).addClass("cqDdDraggable cqDdDraggable_reduced");
        ...
        $(newDrag).attr('questionId', questionId);
        $(newDrag).attr('cqvalue', new Date().getTime());
        $(newDrag).css({
          'width' : '50px',
          'height' : '50px'
        });

        $(newDrag).html('<img src="' + Drupal.settings.closedQuestion.hsimage + '" />');

        /* add it to the answercontainer and set position */
        left = e.pageX - answerContainer.offset().left - $(newDrag).width() / 2;
        top = e.pageY - answerContainer.offset().top - $(newDrag).height() / 2;
        ...
        draggable = $('.cqDdDraggable[cqvalue=' + cqvalue + ']', answerContainer);

        left = e.pageX - answerContainer.offset().left - $(draggable).width() / 2;
        top = e.pageY - answerContainer.offset().top - $(draggable).height() / 2;
        xPos = e.pageX - answerContainer.offset().left;
        yPos = e.pageY - answerContainer.offset().top;
    ...
    $('map[name=' + questionId + 'map]').attr('questionId', questionId);
    $('map[name=' + questionId + 'map]').click(createDraggableFunction);
  }
};
...
function cqCheckAnswerHS(questionId) {
  ...
  $("#" + questionId + "answer").val(returnString);
  return true;
}
