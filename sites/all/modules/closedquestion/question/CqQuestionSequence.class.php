...
class CqQuestionSequence extends CqQuestionAbstract implements CqListenerQuestionInterface {
  ...
  public function getOutput() {
    ...
    if (isset($this->subQuestions[$this->currentIndex])) {
      return theme('closedquestion_question_sequence', $this->subQuestions[$this->currentIndex]->getOutput(), $this->backNext, $this->prefix, $this->postfix);
    }
  }
  ...
  public function getAllText() {
    ...
    foreach ($this->subQuestions as $question) {
      $nr++;
      $retval .= closedquestion_make_fieldset(t('Sub-Question @nr', array('@nr' => $nr)), $question->getAllText());
    }
    return $retval;
  }
  ...
  private function fixBackNextLinks() {
    $prevQuestion = $this->subQuestions[$this->currentIndex - 1];
    $path = $this->usedPath;
    if ($this->currentIndex > 0 && $prevQuestion->isCorrect()) {
      $prevUrl = url($path, array('query' => array('CqQS_' . $this->node->nid . '_Step' => $this->currentIndex - 1)));
    }
    if ($this->currentIndex + 1 < count($this->subQuestions) && $this->subQuestions[$this->currentIndex]->isCorrect()) {
      $nextUrl = url($path, array('query' => array('CqQS_' . $this->node->nid . '_Step' => $this->currentIndex + 1)));
    }
    $this->backNext = theme('closedquestion_sequence_back_next', $this->currentIndex, count($this->subQuestions), $prevUrl, $nextUrl);
